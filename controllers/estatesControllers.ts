...
const getSingleEstateById = catchAsyncErrors(async (req : NextApiRequest, res : NextApiResponse, next : NextFunction) => {
    const estateRes = Array.from(await getOffers()).filter((estate : any) => estate.id == req.query.id);
    let estate;
    if (! estateRes) {
...
const getSingleEstateByLink = catchAsyncErrors(async (req : NextApiRequest, res : NextApiResponse, next : NextFunction) => {
    const estateRes = Array.from(await getOffers()).filter((estate : any) => estate.link == req.query.link);
...
export const getAllEstates = catchAsyncErrors(async (req : NextApiRequest, res : NextApiResponse, next : NextFunction) => {
    try {
        if (req.query.id && req.query.id !== '') {
            ...
            if ('isMore' in req.query && req.query.isMore !== 'false') 
                resPerPage = 12;
             else 
                resPerPage = 8;
            ...
            if (req.query.page) {
                ...
                if (req.query.category) {
                    ...
                    query = estates.filter((estate, index) => index > skip);
                 estates = query;
            }
