...
export const uploadOffersFromEsti = async () : Promise < IEstateFields[] > => {
    const estiOffers = await axios.get(`https://app.esticrm.pl/apiClient/offer/list?company=${
        process.env.ESTICRM_ID
    }&token=${
        process.env.ESTICRM_TOKEN
    }&take=1000`);

    let offers = Array.isArray(estiOffers.data.data) ? estiOffers.data.data : [{
            ... estiOffers.data.data
        }];
    offers = await Promise.all(offers.filter((offer : EstiCRMType) => offer.contactFirstname === 'Marcin' && offer.contactLastname === 'Kumiszczo').map(async (offer : EstiCRMType) => {
        offer.locationStreetName = offer.locationStreetName ? offer.locationStreetName : offer.locationPrecinctName;
        offer.locationPrecinctName = offer.locationParentPrecinctName ? offer.locationParentPrecinctName : offer.locationPrecinctName;
    ...
    offers.forEach((offer : IEstateFields) => {
        const theSameLinkOffers = offers.filter((estioffer : IEstateFields) => { // @ts-ignore
            if (estioffer ?. link) { // @ts-ignore
        ...
        if (theSameLinkOffers.length > 1) {
            theSameLinkOffers.forEach((offer : IEstateFields, index : number) => { // @ts-ignore
                offer.link += index > 1 ? `-${index}` : '';
            });
        }
    });

    offers = await Promise.all(offers.map(async (offer : IEstateFields) => { // @ts-ignore
                ...
                if (isImage) {
                    try {
                        ...
                        return { // @ts-ignore
                            ...
                            src: `${
                                process.env.DOMAIN
                            }/img/offers/simple/${fileName}`,
                            alt: '',
                            dimensions
                        };
                    } catch {return null}} else {
                return null;
            }
        }
    ));
